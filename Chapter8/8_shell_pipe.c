...
{
   pid_t pid;
   char line[256], inputCommand[256], *divcom[32], *arg_c1[32], *arg_c2[32];
   ...
   while (1)
   {
      ...
      if (ret1 > 0)
      {
         ret2 = get_arg2(inputCommand, divcom, "|"); // "|"で区切る
         if (ret2 == 1)
         // inputCommandに"|"がなかった場合
         {
            get_arg2(divcom[0], arg_c1, " "); // " "で区切る
            pid = fork();
            if (pid == 0)
            {
